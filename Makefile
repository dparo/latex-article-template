...
BUILD_DIR:=./build
TARGET_PDF :=$(BUILD_DIR)/out.pdf
OPTIMIZED_PDF :=$(BUILD_DIR)/$(STUDENT_NAME).out.pdf


SVGS := $(shell find ./res -type f -name '*.svg')
SVGS_AS_PDFS := $(patsubst %.svg, %.out.pdf, $(SVGS))
PDFS := $(shell find ./res -type f -name '*.pdf' | grep -v ".cropped.")
...
hard_clean: clean
	...
	find ./res -type f -name "*.out.pdf" -exec rm -rf {} \;
	find ./res -type f -name "*.cropped.pdf" -exec rm -rf {} \;
	rm -rf build
